...
The turtle must be run as `xray X Y Z D C`, where:
...
The turtle automatically broadcasts its location via the ender modem, using its label as a key. You can use `xray-listen` to listen to these messages and log them. You'll probably want to chunk-load the logger.
