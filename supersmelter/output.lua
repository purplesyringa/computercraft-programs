...
local helper_chest_queue_updates = {}
...
function output.flushOutput(immediate)
	...
	util.parForEach(names.all_furnaces, function(furnace)
		...
		if not storage_block_info then
			ok = ok and util.moveItems(furnace, names.output_barrel, 3) >= item.count
			return
		end

		-- The helper chest queues output items that can be converted into storage blocks. If no new
		-- items of a given type arrive for 15s, the slot is flushed to the output barrel. If the
		-- flow is constant, we wait for 9 items to craft into a storage block.
		--
		-- Since the helper chest is constantly flushed due to crafting, there should be enough
		-- space in the allocated slot for the whole output, as long as the turtle doesn't shut down
		-- and the output barrel isn't filled up. If either happens, it'll take multiple iterations
		-- to flush.
		--
		...
		helper_chest_queue_updates[item.name] = now
	end)

	local helper_chest_list = names.helper_chest.list()
	assert(not helper_chest_list[1], "Helper full/precr")
	...
	for _, info in pairs(data.output_storage_blocks) do
		...
		local item = helper_chest_list[info.block_slot]
		if item then
			local count_moved = util.moveItems(
				names.helper_chest,
				names.output_barrel,
		...
		local item = helper_chest_list[info.item_slot]
		if not item then
			goto continue
		end
		...
		if count_recipes > 0 then
			...
			local count_moved = util.moveItems(names.helper_chest, names.output_barrel, 1)
		...
		if not helper_chest_queue_updates[item.name] then
			helper_chest_queue_updates[item.name] = now
		end

		if immediate or helper_chest_queue_updates[item.name] < now - 15 then
